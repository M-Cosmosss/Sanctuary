...
func (n *node) addRoute(path string, handle HandlersChain) (err error) {
	fullPath := path
	n.priority++
...
func (n *node) insertChild(numParams uint8, path, fullPath string, handle HandlersChain) error {
...
// given path.
func (n *node) getValue(path string) (handle HandlersChain, p Params, tsr bool) {
...
func (n *node) findNode(path string) (handle HandlersChain, node *node, tsr bool) {
	var p Params
walk: // outer loop for walking the tree
	for {
		if len(path) > len(n.path) {
